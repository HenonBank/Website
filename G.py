...
# -*- coding: utf-8 -*-
"""
GitHub Script Runner - Сервер для загрузки, установки и запуска Python скриптов
При запуске запрашивает GitHub токен и сразу начинает работу
"""
...
# ============================================================
# НАСТРОЙКИ (заполняются при запуске)
# ============================================================
GITHUB_PAT = None
...
def github_request(method, endpoint, data=None):
    ...
    try:
        ...
        if resp.status_code == 401:
            ...
            return {'error': 'Token lacks permissions (need repo scope)'}
...
def get_github_files():
    """Получение списка .py файлов из GitHub"""
    try:
...
def api_sync():
    ...
    if not GITHUB_PAT:
        return jsonify({"status": "error", "message": "GitHub токен не установлен"}), 400
...
def api_push():
    ...
    try:
        push_to_github(name, content)
        return jsonify({"status": "success", "message": f"Файл {name} отправлен"})
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500
...
""")
    ...
    # Запрашиваем токен
    print("🔑 Введите GitHub Personal Access Token (начинается с ghp_):")
    print("   (токен нужен для доступа к репозиторию)\n")
    
    while not GITHUB_PAT:
        try:
            token = input("➜ ").strip()
            if token:
                # Проверяем формат
                if token.startswith('ghp_') or token.startswith('github_pat_'):
                    ...
                    print("⚠️  Токен должен начинаться с 'ghp_' или 'github_pat_'")
                    print("   Получите токен здесь: https://github.com/settings/tokens\n")
    ...
    test = github_request('GET', f'commits/{GITHUB_BRANCH}')
    if test and isinstance(test, dict) and 'error' in test:
        print(f"❌ Ошибка: {test['error']}")
        print("   Проверьте токен и убедитесь, что у него есть права 'repo'\n")
        sys.exit(1)
    ...
    files = [f for f in os.listdir(SCRIPT_DIR) if f.endswith('.py')]
    print(f"📄 Всего скриптов в папке: {len(files)}")
    print("\n" + "=" * 60)
